...
public final class RequestBypassToServer extends L2GameClientPacket {
  ...
  // FIXME: This is for compatibility, will be changed when bypass functionality got an overhaul by
  // NosBit
  private static final String[] _possibleNonHtmlCommands = {
    "_bbs", "bbs", "_mail", "_friend", "_match", "_diary", "_olympiad?command", "manor_menu_select"
  };
  private final AdminData adminData;
  // S
  private String _command;

  public RequestBypassToServer(AdminData adminData) {
  ...
  protected void runImpl() {
    ...
    if (requiresBypassValidation) {
      ...
      if (bypassOriginId == -1) {
        _log.warning(
            "Player " + activeChar.getName() + " sent non cached bypass: '" + _command + "'");
        return;
      }

      if ((bypassOriginId > 0)
          && !Util.isInsideRangeOfObjectId(
              activeChar, bypassOriginId, L2Npc.INTERACTION_DISTANCE)) {
        // No logging here, this could be a common case where the player has the html still open and
        // run too far away and then clicks a html action
        return;
      }
    }
    ...
    try {
      if (_command.startsWith("admin_")) {
        ...
        try {
          final L2ItemInstance item =
              activeChar.getInventory().getItemByObjectId(Integer.parseInt(id));
        ...
        if (general().allowManor() && (lastNpc != null) && lastNpc.canInteract(activeChar)) {
          ...
          EventDispatcher.getInstance()
              .notifyEventAsync(
                  new OnNpcManorBypass(activeChar, lastNpc, ask, state, time), lastNpc);
        }
      } else {
      ...
      _log.log(
          Level.WARNING,
          "Exception processing bypass from player " + activeChar.getName() + ": " + _command,
          e);
    ...
    EventDispatcher.getInstance()
        .notifyEventAsync(new OnPlayerBypass(activeChar, _command), activeChar);
  }
