...
public class EnterWorld extends L2GameClientPacket {
  ...
  public EnterWorld(PetitionManager petitionManager, AdminData adminData) {
  ...
  protected void runImpl() {
    ...
    if (general().restorePlayerInstance()) {
      activeChar.setInstanceId(
          InstanceManager.getInstance().getPlayerInstance(activeChar.getObjectId()));
    } else {
    ...
    if (general().debug()) {
      if (L2World.getInstance().findObject(activeChar.getObjectId()) != null) {
        _log.warning(
            "User already exists in Object ID map! User "
                + activeChar.getName()
    ...
    if (activeChar.isGM()) {
      if (general().gmStartupInvulnerable()
          && adminData.hasAccess("admin_invul", activeChar.getAccessLevel())) {
        activeChar.setIsInvul(true);
      }

      if (general().gmStartupInvisible()
          && adminData.hasAccess("admin_invisible", activeChar.getAccessLevel())) {
        activeChar.setInvisible(true);
      }

      if (general().gmStartupSilence()
          && adminData.hasAccess("admin_silence", activeChar.getAccessLevel())) {
        activeChar.setSilenceMode(true);
      }

      if (general().gmStartupDietMode()
          && adminData.hasAccess("admin_diet", activeChar.getAccessLevel())) {
      ...
      adminData.addGm(
          activeChar,
          !general().gmStartupAutoList()
              || !adminData.hasAccess("admin_gmliston", activeChar.getAccessLevel()));
    ...
    if (TerritoryWarManager.getInstance().getRegisteredTerritoryId(activeChar) > 0) {
      ...
      activeChar.setSiegeSide(
          TerritoryWarManager.getInstance().getRegisteredTerritoryId(activeChar));
    }

    // Updating Seal of Strife Buff/Debuff
    if (SevenSigns.getInstance().isSealValidationPeriod()
        && (SevenSigns.getInstance().getSealOwner(SEAL_STRIFE) != CABAL_NULL)) {
    ...
    if (vitality().enabled() && vitality().recoverVitalityOnReconnect()) {
      float points =
          (vitality().getRateRecoveryOnReconnect()
                  * (System.currentTimeMillis() - activeChar.getLastAccess()))
              / 60000;
      if (points > 0) {
    ...
    if (showClanNotice) {
      ...
      String serverNews =
          HtmCache.getInstance().getHtm(activeChar.getHtmlPrefix(), "data/html/servnews.htm");
      if (serverNews != null) {
    ...
    if (DimensionalRiftManager.getInstance()
        .checkIfInRiftZone(activeChar.getX(), activeChar.getY(), activeChar.getZ(), false)) {
    ...
    if (!activeChar.canOverrideCond(ZONE_CONDITIONS)
        && activeChar.isInsideZone(SIEGE)
    ...
    if (customs().screenWelcomeMessageEnable()) {
      activeChar.sendPacket(
          new ExShowScreenMessage(
              customs().getScreenWelcomeMessageText(), customs().getScreenWelcomeMessageTime()));
    }
    ...
    if (birthday == 0) {
      ...
      final SystemMessage sm1 =
          SystemMessage.getSystemMessage(THERE_ARE_S1_DAYS_UNTIL_YOUR_CHARACTERS_BIRTHDAY);
      sm1.addInt(birthday);
