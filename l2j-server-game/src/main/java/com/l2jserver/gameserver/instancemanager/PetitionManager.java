...
public class PetitionManager {
  private static final Logger LOG = LogManager.getLogger(PetitionManager.class);
  ...
  public void clearPendingPetitions() {
    final int numPetitions = pendingPetitions.size();
  ...
  public boolean acceptPetition(L2PcInstance respondingAdmin, int petitionId) {
    ...
    Petition currPetition = pendingPetitions.get(petitionId);
  ...
  private boolean isValidPetition(int petitionId) {
    return pendingPetitions.containsKey(petitionId);
  }

  public boolean cancelActivePetition(L2PcInstance player) {
    for (Petition currPetition : pendingPetitions.values()) {
  ...
  public boolean removePetition(int id) {
    return pendingPetitions.remove(id) != null;
  }
  ...
  public boolean endActivePetition(L2PcInstance player) {
    ...
    return pendingPetitions.values().stream()
        ...
        .filter(
            currPetition ->
                (currPetition.getResponder() != null)
                    && (currPetition.getResponder().getObjectId() == player.getObjectId()))
        .findFirst()
        .filter(currPetition -> currPetition.endPetitionConsultation(PetitionState.COMPLETED))
        .isPresent();
  }
  ...
  public int getPlayerTotalPetitionCount(L2PcInstance player) {
    ...
    for (Petition currPetition : completedPetitions.values()) {
      if (currPetition == null) {
        continue;
      }
  ...
  public boolean isPetitionInProcess(int petitionId) {
    ...
    final Petition currPetition = pendingPetitions.get(petitionId);
  ...
  public void sendActivePetitionMessage(L2PcInstance player, String messageText) {
  ...
  public void sendPendingPetitionList(L2PcInstance activeChar) {
    final StringBuilder htmlContent =
        StringUtil.startAppend(
            600 + (getPendingPetitionCount() * 300),
    ...
    for (Petition currPetition : pendingPetitions.values()) {
      ...
      if (currPetition.getState() != PetitionState.IN_PROCESS) {
        StringUtil.append(
            htmlContent,
            "<table width=\"130\" cellpadding=\"2\"><tr>"
        ...
        htmlContent
            .append("<font color=\"")
            .append(currPetition.getResponder().isOnline() ? "00FF00" : "999999")
            .append("\">")
            .append(currPetition.getResponder().getName())
            .append("</font>");
      }
      StringUtil.append(
          htmlContent,
  ...
  public int submitPetition(L2PcInstance petitioner, String petitionText, int petitionType) {
    ...
    final Petition newPetition = new Petition(this, petitioner, petitionText, petitionType);
    int newPetitionId = newPetition.getId();
    pendingPetitions.put(newPetitionId, newPetition);
    ...
    AdminData.getInstance()
        .broadcastToGMs(
            new CreatureSay(
                petitioner.getObjectId(), Say2.HERO_VOICE, "Petition System", msgContent));
