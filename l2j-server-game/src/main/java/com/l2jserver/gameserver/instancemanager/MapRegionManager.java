          ...
          if ("region".equalsIgnoreCase(d.getNodeName())) {
            ...
            for (Node c = d.getFirstChild(); c != null; c = c.getNextSibling()) {
              ...
              if ("respawnPoint".equalsIgnoreCase(c.getNodeName())) {
                ...
                region.addBannedRace(
                    attrs.getNamedItem("race").getNodeValue(),
                    attrs.getNamedItem("point").getNodeValue());
              }
            }
    ...
    if (activeChar.isPlayer()) {
      ...
      if ((player.getClan() != null) && !player.isFlyingMounted() && !player.isFlying()) {
        ...
        if (teleportWhere == TeleportWhereType.CLANHALL) {
          final var clanhall =
              ClanHallManager.getInstance().getAbstractHallByOwner(player.getClan());
          if (clanhall != null) {
        ...
        if (teleportWhere == TeleportWhereType.SIEGEFLAG) {
          ...
          L2SiegeFlagInstance tw_flag =
              TerritoryWarManager.getInstance().getHQForClan(player.getClan());
          if (tw_flag != null) {
