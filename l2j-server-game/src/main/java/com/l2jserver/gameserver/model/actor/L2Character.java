...
import com.l2jserver.gameserver.model.actor.tasks.character.*;
...
import com.l2jserver.gameserver.model.skills.*;
...
public abstract class L2Character extends L2Object implements ISkillsHolder, IDeletable {
	...
	public CharEffectList getEffectList() {
		return _effectList;
	}
	...
	private void beginCast(Skill skill, boolean simultaneously) {
		...
		List<L2Object> targets = skill.getTargetList(this);
		...
		switch (skill.getTargetType()) {
			...
			default:
				...
				if (doit) {
					target = (L2Character) targets.get(0);
				} else {
	...
	public void onMagicLaunchedTimer(MagicUseTask mut) {
		...
		if ((targets.size() > 0) && (escapeRange > 0)) {
			...
			mut.setTargets(List.copyOf(targetList));
		}
	...
	public void onMagicFinalizer(MagicUseTask mut) {
		...
		final Skill skill = mut.getSkill();
		final L2Object target = mut.getTargets().stream().findFirst().orElse(null);
	...
	public void makeTriggerCast(Skill skill, L2Character target, boolean ignoreTargetType) {
		try {
			...
			if (skill.checkCondition(this, target, false)) {
				...
				final List<L2Object> targets = !ignoreTargetType ? skill.getTargetList(this, false, target) : List.of( target );
				if (targets.isEmpty()) {
					return;
				}
