  ...
  public void run() {
    ...
    try {
      ...
      if (!AttackStanceTaskManager.getInstance().hasAttackStanceTask(_cubic.getOwner())) {
        if (_cubic.getOwner().hasSummon()) {
          if (!AttackStanceTaskManager.getInstance()
              .hasAttackStanceTask(_cubic.getOwner().getSummon())) {
            _cubic.stopAction();
            return;
          }
        } else {
      ...
      if (useCubicCure) {
        ...
        MagicSkillUse msu =
            new MagicSkillUse(
                _cubic.getOwner(), _cubic.getOwner(), L2CubicInstance.SKILL_CUBIC_CURE, 1, 0, 0);
        ...
        if ((target != null) && !target.isDead()) {
          if (general().debug()) {
            ...
            _log.info(
                "Cubic ID: "
                    ...
                    + target.getName()
                    + " distance: "
                    + target.calculateDistance(_cubic.getOwner(), true, false));
          }
          ...
          _cubic
              ...
              .broadcastPacket(
                  new MagicSkillUse(
                      _cubic.getOwner(), target, skill.getId(), skill.getLevel(), 0, 0));

          List<L2Character> targets = List.of(target);
          ...
          } else if (skill.hasEffectType(
              L2EffectType.STUN, L2EffectType.ROOT, L2EffectType.PARALYZE)) {
            if (general().debug()) {
              _log.info("L2CubicInstance: Action.run() skill " + skill);
            }
