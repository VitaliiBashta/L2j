...
public class QuestState {
    ...
    public String set(String var, String val) {
        ...
        if ("cond".equals(var)) {
            try {
                ...
                _log.log(Level.WARNING, player.getName() + ", " + getQuestName() + " cond [" + val + "] is not an integer.  Value stored, but no packet was sent: " + e.getMessage(), e);
            }
        }
    ...
    public int getInt(String var) {
        ...
        try {
            ...
            _log.log(Level.INFO, "Quest " + getQuestName() + ", method getInt(" + var + "), tried to parse a non-integer value (" + variable + "). Char Id: " + player.getObjectId(), nfe);
        }
    ...
    public long getQuestItemsCount(int itemId) {
        return AbstractScript.getQuestItemsCount(player, itemId);
    }
    ...
    public boolean hasQuestItems(int itemId) {
        return AbstractScript.hasQuestItems(player, itemId);
    }
    ...
    public boolean hasQuestItems(int... itemIds) {
        return AbstractScript.hasQuestItems(player, itemIds);
    }
    ...
    public int getEnchantLevel(int itemId) {
        return AbstractScript.getEnchantLevel(player, itemId);
    }
    ...
    public void giveItems(int itemId, long count, int enchantlevel) {
        AbstractScript.giveItems(player, itemId, count, enchantlevel);
    }
    ...
    public void rewardItems(ItemHolder item) {
        AbstractScript.rewardItems(player, item);
    }
    ...
    public void rewardItems(int itemId, long count) {
        AbstractScript.rewardItems(player, itemId, count);
    }
    ...
    public void giveItems(ItemHolder holder) {
        AbstractScript.giveItems(player, holder.getId(), holder.getCount(), 0);
    }

    public void giveItems(int itemId, long count, byte attributeId, int attributeLevel) {
        AbstractScript.giveItems(player, itemId, count, attributeId, attributeLevel);
    }

    public boolean giveItemRandomly(int itemId, long amount, long limit, double dropChance, boolean playSound) {
        return AbstractScript.giveItemRandomly(player, null, itemId, amount, amount, limit, dropChance, playSound);
    }

    public boolean giveItemRandomly(L2Npc npc, int itemId, long amount, long limit, double dropChance, boolean playSound) {
        return AbstractScript.giveItemRandomly(player, npc, itemId, amount, amount, limit, dropChance, playSound);
    }
    ...
    public boolean giveItemRandomly(L2Npc npc, int itemId, long minAmount, long maxAmount, long limit, double dropChance, boolean playSound) {
        return AbstractScript.giveItemRandomly(player, npc, itemId, minAmount, maxAmount, limit, dropChance, playSound);
    }
    ...
    public L2Npc addSpawn(int npcId, int despawnDelay) {
        return addSpawn(npcId, player.getX(), player.getY(), player.getZ(), 0, false, despawnDelay, false);
    }
