...
public class PcInventory extends Inventory {
   ...
   */
  @Override
  public L2ItemInstance dropItem(
    ...
    if (item != null) {
      EventDispatcher.getInstance()
          .notifyEventAsync(new OnPlayerItemDrop(actor, item, item.getLocation()), item.getItem());
    }
    return item;
  }
  ...
  @Override
  public L2ItemInstance dropItem(
      String process, int objectId, long count, L2PcInstance actor, Object reference) {
  ...
  public L2ItemInstance addItem(
    ...
    if (item != null) {
      // Notify to scripts
      EventDispatcher.getInstance()
          .notifyEventAsync(new OnPlayerItemAdd(actor, item), actor, item.getItem());
    }
    return item;
  }
   ...
   * @param enchantLevel : int Enchant of the item; -1 to not modify on existing items, for new
   *     items use the default enchantLevel when -1
  ...
  public L2ItemInstance addItem(
    ...
    final L2ItemInstance item =
        super.addItem(process, itemId, count, enchantLevel, actor, reference);
    if (item != null) {
  ...
  public L2ItemInstance transferItem(
    ...
    // Notify to scripts
    EventDispatcher.getInstance()
        .notifyEventAsync(new OnPlayerItemTransfer(actor, item, target), item.getItem());
    return item;
  }
  ...
  public L2ItemInstance destroyItem(
    ...
    if (item != null) {
      EventDispatcher.getInstance()
          .notifyEventAsync(new OnPlayerItemDestroy(actor, item), item.getItem());
    }
    return item;
  }
   ...
   */
  @Override
  public L2ItemInstance destroyItemByItemId(
  ...
  public L2ItemInstance[] getUniqueItems(
      boolean allowAdena, boolean allowAncientAdena, boolean onlyAvailable) {
    ...
    for (L2ItemInstance item : _items) {
      ...
      if (!isDuplicate
          && (!onlyAvailable
              || (item.isSellable() && item.isAvailable(getOwner(), false, false)))) {
        list.add(item);
      }
    }
  ...
  /**
   * Returns the list of items in inventory available for transaction Allows an item to appear twice
   * if and only if there is a difference in enchantment level.
   *
   * @return L2ItemInstance : items in inventory
   */
  public L2ItemInstance[] getUniqueItemsByEnchantLevel(
      boolean allowAdena, boolean allowAncientAdena) {
  ...
  /**
   * Returns the list of all items in inventory that have a given item id AND a given enchantment
   * level.
   *
   * @param itemId : ID of item
   ...
   * @return L2ItemInstance[] : matching items from inventory
   */
  public L2ItemInstance[] getAllItemsByItemId(
      int itemId, int enchantment, boolean includeEquipped) {
    List<L2ItemInstance> list = new LinkedList<>();
    for (L2ItemInstance item : _items) {
      if (item == null) {
        continue;
  ...
  public L2ItemInstance[] getAvailableItems(
      boolean allowAdena, boolean allowNonTradeable, boolean feightable) {
    List<L2ItemInstance> list = new LinkedList<>();
    for (L2ItemInstance item : _items) {
      if ((item == null)
          || !item.isAvailable(getOwner(), allowAdena, allowNonTradeable)
          || !canManipulateWithItemId(item.getId())) {
        continue;
      } else if (feightable) {
  ...
  public boolean reduceAncientAdena(
      String process, long count, L2PcInstance actor, Object reference) {
    if (count > 0) {
   ...
   * @return {@code true} if the inventory isn't full after taking new items and items weight add to
   *     current load doesn't exceed max weight load.
   */
  public boolean checkInventorySlotsAndWeight(
      List<L2Item> itemList, boolean sendMessage, boolean sendSkillMessage) {
    int lootWeight = 0;
    int requiredSlots = 0;
    if (itemList != null) {
      for (L2Item item : itemList) {
        // If the item is not stackable or is stackable and not present in inventory, will need a
        // slot.
