...
public final class L2Weapon extends L2Item {
  ...
  public L2Weapon(StatsSet set) {
    ...
    String[] damgeRange =
        set.getString("damage_range", "").split(";"); // 0?;0?;fan sector;base attack angle
    ...
    _reducedSoulshotChance =
        (reduced_soulshots.length == 2) ? Integer.parseInt(reduced_soulshots[0]) : 0;
    ...
    _reducedMpConsumeChance =
        (reduced_mpconsume.length == 2) ? Integer.parseInt(reduced_mpconsume[0]) : 0;
    _reducedMpConsume =
        (reduced_mpconsume.length == 2) ? Integer.parseInt(reduced_mpconsume[1]) : 0;
    ...
    if (skill != null) {
      ...
      if (info.length == 2) {
        ...
        try {
          ...
          _log.info(
              StringUtil.concat(
                  "> Couldnt parse ", skill, " in weapon enchant skills! item ", toString()));
        }
        if ((id > 0) && (level > 0)) {
    ...
    if (skill != null) {
      ...
      if (info.length == 2) {
        ...
        try {
          ...
          _log.info(
              StringUtil.concat(
                  "> Couldnt parse ", skill, " in weapon onmagic skills! item ", toString()));
        }
    ...
    if (skill != null) {
      ...
      if (info.length == 2) {
        ...
        try {
          ...
          _log.info(
              StringUtil.concat(
                  "> Couldnt parse ", skill, " in weapon oncrit skills! item ", toString()));
        }
        if ((id > 0) && (level > 0) && (chance > 0)) {
  ...
  public void castOnMagicSkill(L2Character caster, L2Character target, Skill trigger) {
    ...
    if (onMagicSkill.isBad()
        && (Formulas.calcShldUse(caster, target, onMagicSkill)
    ...
    onMagicSkill.activateSkill(caster, List.of(target));
          ...
          .forEach(
              npc ->
                  EventDispatcher.getInstance()
                      .notifyEventAsync(
                          new OnNpcSkillSee(
                              (L2Npc) npc, caster.getActingPlayer(), onMagicSkill, targets, false),
                          npc));
    }
    if (caster.isPlayer()) {
