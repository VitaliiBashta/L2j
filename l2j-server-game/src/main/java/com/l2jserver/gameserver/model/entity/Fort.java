...
public final class Fort extends AbstractResidence {
  ...
  public boolean setOwner(L2Clan clan, boolean updateClansReputation) {
    ...
    if (clan.getCastleId() > 0) {
      getSiege()
          .announceToPlayer(
              SystemMessage.getSystemMessage(SystemMessageId.NPCS_RECAPTURED_FORTRESS));
      return false;
    }
  ...
  protected void load() {
    try (var con = ConnectionFactory.getInstance().getConnection();
      ...
      if (ownerId > 0) {
        ...
        setOwnerClan(clan);
        int runCount =
            getOwnedTime() / (int) MILLISECONDS.toSeconds(fortress().getPeriodicUpdateFrequency());
        ...
        if ((fortress().getMaxKeepTime() <= 0)
            || (getOwnedTime() < (fortress().getMaxKeepTime() * 3600))) {
          fortUpdater[0] =
  ...
  public int getTimeTillRebelArmy() {
    ...
    return (int)
        (((_lastOwnedTime.getTimeInMillis() + (fortress().getMaxKeepTime() * 3600000L))
   ...
   *
   * @return {@code true} if this is a border fortress (associated with more than one castle),
  ...
  private void initSiegeNpcs() {
    ...
    try (var con = ConnectionFactory.getInstance().getConnection();
      ...
      _log.log(
          ...
          "Fort "
              + getResidenceId()
              + " initSiegeNpcs: Spawn could not be initialized: "
              + e.getMessage(),
          e);
    }
  }
  ...
  private void initSpecialEnvoys() {
    ...
    try (var con = ConnectionFactory.getInstance().getConnection();
      ...
      _log.log(
          ...
          "Fort "
              + getResidenceId()
              + " initSpecialEnvoys: Spawn could not be initialized: "
              + e.getMessage(),
          e);
    }
  }
  ...
  public class FortFunction {
    ...
    public FortFunction(
        int type, int lvl, int lease, int tempLease, long rate, long time, boolean cwh) {
      _type = type;
      _lvl = lvl;
      _fee = lease;
      _tempFee = tempLease;
