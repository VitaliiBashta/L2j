...
public final class L2TamedBeastInstance extends L2FeedableBeastInstance {
  ...
  public L2TamedBeastInstance(
      int npcTemplateId, L2PcInstance owner, int foodSkillId, int x, int y, int z) {
    super(NpcData.getInstance().getTemplate(npcTemplateId));
    _isFreyaBeast = false;
    setInstanceType(InstanceType.L2TamedBeastInstance);
  ...
  public L2TamedBeastInstance(
      int npcTemplateId, L2PcInstance owner, int food, int x, int y, int z, boolean isFreyaBeast) {
    super(NpcData.getInstance().getTemplate(npcTemplateId));
      ...
      _durationCheckTask =
          ThreadPoolManager.getInstance()
              .scheduleGeneralAtFixedRate(
                  new CheckDuration(this), DURATION_CHECK_INTERVAL, DURATION_CHECK_INTERVAL);
    }
  }
    ...
    if (owner != null) {
      ...
      if (!_isFreyaBeast) {
        ...
        for (Skill skill : getTemplate().getSkills().values()) {
          // if the skill is a buff, check if the owner has it already [ owner.getEffect(L2Skill
          // skill) ]
  ...
  public void onOwnerGotAttacked(L2Character attacker) {
    ...
    // if the owner has a lot of HP, then debuff the enemy with a random debuff among the available
    // skills
    // use of more than one debuff at this moment is acceptable
    if (HPRatio >= 0.8) {
      for (Skill skill : getTemplate().getSkills().values()) {
        // if the skill is a debuff, check if the attacker has it already [
        // attacker.getEffect(L2Skill skill) ]
        if (skill.isDebuff()
            && (Rnd.get(3) < 1)
            && ((attacker != null) && attacker.isAffectedBySkill(skill.getId()))) {
          sitCastAndFollow(skill, attacker);
        }
      }
    }
    // for HP levels between 80% and 50%, do not react to attack events (so that MP can regenerate a
    // bit)
    // for lower HP ranges, heal or recharge the owner with 1 skill use per attack.
      ...
      // if the owner has a lot of HP, then debuff the enemy with a random debuff among the
      // available skills
      for (Skill skill : getTemplate().getSkills().values()) {
        // if the skill is a buff, check if the owner has it already [ owner.getEffect(L2Skill
        // skill) ]
  ...
  private static class CheckDuration implements Runnable {
    ...
    public void run() {
      ...
      if (_tamedBeast._isFreyaBeast) {
        ...
        // if the owner has enough food, call the item handler (use the food and triffer all
        // necessary actions)
        if ((item != null) && (item.getCount() >= 1)) {
          L2Object oldTarget = owner.getTarget();
          owner.setTarget(_tamedBeast);
          List<L2Object> targets = List.of(_tamedBeast);

          // emulate a call to the owner using food, but bypass all checks for range, etc
          // this also causes a call to the AI tasks handling feeding, which may call onReceiveFood
          // as required.
        ...
        // There are too many conflicting reports about whether distance from home should be taken
        // into consideration. Disabled for now.
  ...
  private class CheckOwnerBuffs implements Runnable {
    ...
    public void run() {
      ...
      // if the tamed beast is currently casting a spell, do not interfere (do not attempt to cast
      // anything new yet).
      if (isCastingNow()) {
        return;
      }
