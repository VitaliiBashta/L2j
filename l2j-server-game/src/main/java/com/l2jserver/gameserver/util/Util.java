...
public final class Util {
  ...
  public static void handleIllegalPlayerAction(
      L2PcInstance actor, String message, IllegalActionPunishmentType punishment) {
    ThreadPoolManager.getInstance()
        .scheduleGeneral(new IllegalPlayerActionTask(actor, message, punishment), 5000);
  }

  /** degree value of object 2 to the horizontal line with object 1 being the origin. */
  ...
  public static double calculateDistance(
    ...
    final double distance =
        Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2) + (includeZAxis ? Math.pow(z1 - z2, 2) : 0);
   ...
   * @return the value of {@code number} rounded to specified number of digits after the decimal
   *     point.
   */
  ...
  /**
   * @param text - the text to check
   * @return {@code true} if {@code text} contains only letters and/or numbers, {@code false}
   *     otherwise
   */
  ...
  private static void buildHtmlBypassCache(
      L2PcInstance player, HtmlActionScope scope, String html) {
    ...
    activeChar.sendPacket(
        new ShowBoard(
            Arrays.asList(
                ...
                " ",
                " ",
                text.replaceAll("<br>", Configuration.EOL),
                "0",
                "0",
                "0",
                "0")));
  ...
  public static boolean isInsideRangeOfObjectId(L2Object obj, int targetObjId, int radius) {
    ...
    double distance = obj.calculateDistance(target, false, false);
  ...
  public static double map(
      double input, double inputMin, double inputMax, double outputMin, double outputMax) {
    input = constrain(input, inputMin, inputMax);
    return (((input - inputMin) * (outputMax - outputMin)) / (inputMax - inputMin)) + outputMin;
  }
