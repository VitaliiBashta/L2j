...
public class PostBBSManager extends BaseBBSManager {
  ...
  private final Map<Topic, List<Post>> postByTopic = new ConcurrentHashMap<>();
  ...
  public List<Post> getGPostByTopic(Topic topic) {
    var posts = postByTopic.get(topic);
    if (posts == null) {
      posts = DAOFactory.getInstance().getPostRepository().load(topic);
      postByTopic.put(topic, posts);
    }
    return posts;
  }
  ...
  public void addPostByTopic(Topic topic, List<Post> posts) {
    postByTopic.putIfAbsent(topic, posts);
  }
  ...
  private void showEditPost(Topic topic, Forum forum, L2PcInstance activeChar, int idp) {
    if (topic == null) {
      CommunityBoardHandler.separateAndSend(
          "<html><body><br><br><center>Error: This topic does not exist!</center></body></html>",
          activeChar);
    } else {
      final List<Post> p = getGPostByTopic(topic);
      if ((forum == null) || (p == null)) {
        CommunityBoardHandler.separateAndSend(
            "<html><body><br><br><center>Error: This forum or post does not exist!</center></body></html>",
            activeChar);
      } else {
  ...
  private void showPost(Topic topic, Forum forum, L2PcInstance activeChar, int ind) {
    if ((forum == null) || (topic == null)) {
      CommunityBoardHandler.separateAndSend(
          "<html><body><br><br><center>Error: This forum is not implemented yet!</center></body></html>",
          activeChar);
    } else if (forum.getType() == ForumType.MEMO) {
      showMemoPost(topic, activeChar, forum);
    } else {
      CommunityBoardHandler.separateAndSend(
          "<html><body><br><br><center>The forum: "
              + forum.getName()
              + " is not implemented yet!</center></body></html>",
          activeChar);
    }
  }

  private void showHtmlEditPost(
      Topic topic, L2PcInstance activeChar, Forum forum, List<Post> posts) {
    final String html =
        StringUtil.concat(
  ...
  private void showMemoPost(Topic topic, L2PcInstance activeChar, Forum forum) {
    ...
    final String html =
        StringUtil.concat(
            ...
            dateFormat.format(posts.get(0).getDate()),
  ...
  public void parsewrite(
    ...
    if (f == null) {
      ...
      if (t == null) {
        CommunityBoardHandler.separateAndSend(
            "<html><body><br><br><center>the topic: "
                + idt
                + " does not exist !</center><br><br></body></html>",
            activeChar);
      } else {
        ...
        if (!posts.isEmpty()) {
          ...
          if (post == null) {
            CommunityBoardHandler.separateAndSend(
                "<html><body><br><br><center>the post: "
                    + idp
                    + " does not exist !</center><br><br></body></html>",
                activeChar);
          } else {
