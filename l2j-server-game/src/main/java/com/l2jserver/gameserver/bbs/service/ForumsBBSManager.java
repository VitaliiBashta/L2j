...
public class ForumsBBSManager {
  ...
  public void load() {
    ...
    FORUMS_BY_ID.putAll(
        FORUMS_BY_NAME.values().stream().collect(Collectors.toMap(Forum::getId, f -> f)));
  ...
  public Forum create(
      String name, Forum parent, ForumType type, ForumVisibility visibility, int ownerId) {
  ...
  public Forum load(
      int id, String name, Forum parent, ForumType type, ForumVisibility visibility, int ownerId) {
      ...
      if (clanRootForum != null) {
        ...
        if (forum == null) {
          ForumsBBSManager.getInstance()
              .create(clan.getName(), clanRootForum, CLAN, CLAN_MEMBER_ONLY, clan.getId());
        }
      }
    }
  }
