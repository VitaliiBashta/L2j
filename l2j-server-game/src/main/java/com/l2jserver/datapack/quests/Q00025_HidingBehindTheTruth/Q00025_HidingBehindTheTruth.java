...
public class Q00025_HidingBehindTheTruth extends Quest {
  ...
  public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon) {
    if (npc.getCurrentHp() <= (0.30 * npc.getMaxHp())) {
      ...
      if (qs.isMemoState(8)
              && !hasQuestItems(attacker, TOTEM_DOLL3)
              && (attacker.getObjectId() == npc.getScriptValue())) {
        ...
        npc.broadcastPacket(
                new NpcSay(
                        ...
                        npc.getId(),
                        NpcStringId
                                .YOUVE_ENDED_MY_IMMORTAL_LIFE_YOURE_PROTECTED_BY_THE_FEUDAL_LORD_ARENT_YOU));
  ...
  public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
    ...
    switch (event) {
      ...
      case "31349-03.html": {
        if (qs.isCreated()
                && player.hasQuestCompleted(
                Q00024_InhabitantsOfTheForestOfTheDead.class.getSimpleName())
      ...
      case "SAY_TRIYOL": {
        npc.broadcastPacket(
                new NpcSay(
                        ...
                        Say2.ALL,
                        npc.getId(),
                        NpcStringId.THAT_BOX_WAS_SEALED_BY_MY_MASTER_S1_DONT_TOUCH_IT)
    ...
    switch (qs.getState()) {
      case State.CREATED: {
        if (npc.getId() == PRIEST_BENEDICT) {
          QuestState q24 =
                  talker.getQuestState(Q00024_InhabitantsOfTheForestOfTheDead.class.getSimpleName());
