...
public class Q00350_EnhanceYourWeapon extends Quest {
  ...
  public String onTalk(L2Npc npc, L2PcInstance player) {
    ...
    } else if (!st.hasQuestItems(RED_SOUL_CRYSTAL0_ID)
        && !st.hasQuestItems(GREEN_SOUL_CRYSTAL0_ID)
        && !st.hasQuestItems(BLUE_SOUL_CRYSTAL0_ID)) {
  ...
  private static void exchangeCrystal(
      L2PcInstance player, L2Attackable mob, int takeid, int giveid, boolean broke) {
    L2ItemInstance Item =
        player.getInventory().destroyItemByItemId("SoulCrystal", takeid, 1, player, mob);
    if (Item != null) {
  ...
  /**
   * Calculate the leveling chance of Soul Crystals based on the attacker that killed this
   * L2Attackable
   *
   * @param mob
   * @param killer The player that last killed this L2Attackable $ Rewrite 06.12.06 - Yesod $
  ...
  public static void levelSoulCrystals(L2Attackable mob, L2PcInstance killer) {
    ...
    if (isPartyLevelingMonster(mob.getId()) && (killer.getParty() != null)) {
      ...
      for (L2PcInstance pl : killer.getParty().getMembers()) {
        ...
        if ((maxSCLevel < sc.getLevel())
            && NPC_LEVELING_INFO.get(mob.getId()).containsKey(sc.getLevel())) {
    ...
    if (mainlvlInfo.isSkillNeeded()) {
      ...
      // Check if the soul crystal was used when HP of this L2Attackable wasn't higher than half of
      // it
    ...
    switch (mainlvlInfo.getAbsorbCrystalType()) {
      case PARTY_ONE_RANDOM:
        ...
        // among those who have crystals, only. However, this might actually be correct (same as
        // retail).
        if (killer.getParty() != null) {
          L2PcInstance lucky =
              killer.getParty().getMembers().get(getRandom(killer.getParty().getMemberCount()));
          ...
          if ("crystal".equalsIgnoreCase(n.getNodeName())) {
            for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling()) {
              if ("item".equalsIgnoreCase(d.getNodeName())) {
                ...
                int leveledItemId =
                    Integer.parseInt(attrs.getNamedItem("leveledItemId").getNodeValue());
                ...
                for (Node cd = d.getFirstChild(); cd != null; cd = cd.getNextSibling()) {
                  ...
                  if ("detail".equalsIgnoreCase(cd.getNodeName())) {
                    ...
                    if ((att1 == null) && (att2 == null)) {
                      LOG.error(
                          "Missing maxlevel/levelList in NPC List npc Id {}, skipping!", npcId);
                      continue;
                    }
