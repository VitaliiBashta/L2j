...
public class TerritoryWarSuperClass extends Quest {
  ...
  public TerritoryWarSuperClass(int questId, String name, String descr) {
    ...
    if (questId < 0) {
      ...
      final long nextSiegeDate =
          GlobalVariablesManager.getInstance().getLong(TerritoryWarManager.GLOBAL_VARIABLE, 0);
      ...
      LOG.info(getClass().getSimpleName() + ": Siege date: " + cal.getTime());
    }
  }
  ...
  private static void handleBecomeMercenaryQuest(L2PcInstance player, boolean catapult) {
    ...
    QuestState st =
        player.getQuestState(Q00147_PathtoBecominganEliteMercenary.class.getSimpleName());
  ...
  public static void main(String[] args) {
    ...
    new TerritoryWarSuperClass(
        -1, TerritoryWarSuperClass.class.getSimpleName(), "Territory War Superclass");
    ...
    if ((actingPlayer != null) && (qs.getPlayer() != null)) {
      if (actingPlayer.getParty() != null) {
        for (L2PcInstance pl : actingPlayer.getParty().getMembers()) {
          if ((pl.getSiegeSide() == qs.getPlayer().getSiegeSide())
              || (pl.getSiegeSide() == 0)
      ...
      } else if ((actingPlayer.getSiegeSide() != qs.getPlayer().getSiegeSide())
          && (actingPlayer.getSiegeSide() > 0)) {
    ...
    if (territoryId > 0) {
      ...
      if (player.getLevel() >= 61) {
        ...
        if (killthe != null) {
          ...
          LOG.warn(
              "TerritoryWar: Missing Kill the quest for player "
                  + player.getName()
                  + " whose class id: "
  ...
  public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon) {
    ...
    if (npc.getId() == CATAPULT_ID) {
      ...
      manager.announceToParticipants(
          new ExShowScreenMessage(npcString[0], 2, 10000), 135000, 13500);
    ...
    if (targets.contains(npc)) {
      if (skill.getId() == 845) {
        ...
        if ((caster.getSiegeSide() - 80) == ward.getOwnerCastleId()) {
          for (TerritoryNPCSpawn wardSpawn :
              TerritoryWarManager.getInstance()
                  .getTerritory(ward.getOwnerCastleId())
                  .getOwnedWard()) {
          ...
          ward.setNpc(
              TerritoryWarManager.getInstance()
                  .addTerritoryWard(
                      ward.getTerritoryId(),
                      caster.getSiegeSide() - 80,
                      ward.getOwnerCastleId(),
                      true));
          ward.setOwnerCastleId(caster.getSiegeSide() - 80);
          TerritoryWarManager.getInstance()
              .getTerritory(caster.getSiegeSide() - 80)
              .getQuestDone()[1]++;
        }
      }
    }
  ...
  public void setOnEnterWorld(boolean val) {
    ...
    for (L2PcInstance player : L2World.getInstance().getPlayers()) {
      if (player.getSiegeSide() > 0) {
        ...
        QuestState st =
            player.hasQuestState(territoryQuest.getName())
