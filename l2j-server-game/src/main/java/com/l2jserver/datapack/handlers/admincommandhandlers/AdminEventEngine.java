...
public class AdminEventEngine implements IAdminCommandHandler {
  ...
  public boolean useAdminCommand(String command, L2PcInstance activeChar) {
    ...
    try {
      if (actualCommand.equals("admin_event")) {
        ...
        // There is an exception here for not using the ST. We use spaces (ST delim) for the event
        // info.
        ...
        try {
          ...
          try (FileInputStream fis =
                  new FileInputStream(server().getDatapackRoot() + "/data/events/" + eventName);
            ...
            adminReply.replace(
                "npc_%objectId%_event_participate",
                "admin_event"); // Weird, but nice hack, isnt it? :)
        ...
        try {
          try (FileOutputStream file =
                  new FileOutputStream(
                      new File(server().getDatapackRoot(), "data/events/" + tempName));
        ...
        Broadcast.toAllOnlinePlayers(
            activeChar.getName()
                + " has started an event. You will find a participation NPC somewhere around you.");
        ...
        for (L2PcInstance player : L2Event._teams.get(teamId)) {
          ...
          if (!TransformData.getInstance().transformPlayer(transId, player)) {
            AdminData.getInstance()
                .broadcastMessageToGMs("EventEngine: Unknow transformation id: " + transId);
          }
        }
        showEventControl(activeChar);
      ...
      AdminData.getInstance()
          .broadcastMessageToGMs(
              "EventEngine: Error! Possible blank boxes while executing a command which requires a value in the box?");
    }
    return true;
  }
  ...
  private String showStoredEvents() {
    ...
    if (dir.isFile()) {
      return "<font color=\"FF0000\">The directory '"
          + dir.getAbsolutePath()
          + "' is a file or is corrupted!</font><br>";
    }

    String note = "";
    if (!dir.exists()) {
      note =
          "<font color=\"FF0000\">The directory '"
              + dir.getAbsolutePath()
              + "' does not exist!</font><br><font color=\"0099FF\">Trying to create it now...<br></font><br>";
      if (dir.mkdirs()) {
        note +=
            "<font color=\"006600\">The directory '"
                + dir.getAbsolutePath()
                + "' has been created!</font><br>";
      } else {
        note +=
            "<font color=\"FF0000\">The directory '"
                + dir.getAbsolutePath()
                + "' hasn't been created!</font><br>";
        return note;
      }
    }
  ...
  public void showNewEventPage(L2PcInstance activeChar) {
    ...
    replyMSG.append(
        "<center><multiedit var=\"txt\" width=270 height=100> <button value=\"Add text\" action=\"bypass -h admin_add $txt\" width=120 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
    replyMSG.append(
    ...
    if (!(tempName.isEmpty() && tempBuffer.isEmpty())) {
      replyMSG.append(
          "<br><button value=\"Store Event Data\" action=\"bypass -h admin_event_store\" width=160 height=32 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
    }
  ...
  public void showEventParameters(L2PcInstance activeChar, int teamnumbers) {
    ...
    sb.append(
        "<html><body><title>[ L2J EVENT ENGINE ]</title><br><center> Current event: <font color=\"LEVEL\">");
    sb.append(L2Event._eventName);
    sb.append(
        "</font></center><br>INFO: To start an event, you must first set the number of teams, then type their names in the boxes and finally type the NPC ID that will be the event manager (can be any existing npc) next to the \"Announce Event!\" button.<br><table width=100%>");
    sb.append(
    ...
    sb.append(
        "\" width=140 height=32 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
  ...
  private void showEventControl(L2PcInstance activeChar) {
    ...
    StringBuilder sb = new StringBuilder();
    sb.append(
        "<html><title>[ L2J EVENT ENGINE ]</title><body><br><center>Current event: <font color=\"LEVEL\">");
    sb.append(L2Event._eventName);
    sb.append(
    ...
    sb.append(
        "<tr><td>&nbsp;</td></tr>"
            ...
            + "<tr><td>ID</td><td><edit var=\"id\" width=60 height=15></td></tr></table></td><td><font color=\"LEVEL\">Give the specified item id to every single member of the team, you can put 5*level, 5*kills or 5 in the number field for example</font></td></tr>"
            + "<tr><td>&nbsp;</td></tr>"
            ...
            + "<td><font color=\"LEVEL\">Kicks the specified player(s) from the event. Blank field kicks target.</font></td></tr>"
            + "<tr><td>&nbsp;</td></tr>"
  ...
  private void rewardTeam(L2PcInstance activeChar, int team, int n, int id, String type) {
    ...
    for (L2PcInstance player : L2Event._teams.get(team)) {
      ...
      final NpcHtmlMessage adminReply = new NpcHtmlMessage();
      adminReply.setHtml(
          "<html><body> CONGRATULATIONS! You should have been rewarded. </body></html>");
