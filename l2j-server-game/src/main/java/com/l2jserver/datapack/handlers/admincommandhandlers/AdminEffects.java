  ...
  public boolean useAdminCommand(String command, L2PcInstance activeChar) {
    ...
    if (command.equals("admin_invis_menu")) {
      ...
      activeChar.sendMessage(
          "You've made "
              + target.getName()
              + " "
              + (target.isInvisible() ? "invisible" : "visible")
              + ".");
      ...
      try {
        ...
        Earthquake eq =
            new Earthquake(
                activeChar.getX(), activeChar.getY(), activeChar.getZ(), intensity, duration);
      ...
      try {
        ...
        activeChar.sendMessage(
            "Usage: //atmosphere <signsky dawn|dusk>|<sky day|night|red> <duration>");
      }
    ...
    } else if (command.startsWith("admin_unpara")) // || command.startsWith("admin_unpara_menu"))
    {
      ...
      try {
        ...
        || command.startsWith("admin_ave_special")
        || command.startsWith("admin_ave_event")) {
      if (st.countTokens() > 0) {
        ...
        if (radius > 0) {
          ...
          activeChar.sendMessage(
              "Affected all characters in radius "
                  + param2
                  + " by "
                  + param1
          ...
          if (performAbnormalVisualEffect(ave, obj)) {
            activeChar.sendMessage(
                obj.getName() + " affected by " + param1 + " abnormal visual effect.");
          } else {
            activeChar.sendPacket(SystemMessageId.NOTHING_HAPPENED);
          }
        }
        ...
        activeChar.sendMessage(
            "Usage: //" + command.replace("admin_", "") + " <AbnormalVisualEffect> [radius]");
      }
    ...
    try {
      if (target instanceof L2Character) {
        ...
        if ((target instanceof L2PcInstance)
            && ((action < 2) || ((action > 18) && (action != SocialAction.LEVEL_UP)))) {
          activeChar.sendPacket(SystemMessageId.NOTHING_HAPPENED);
          return false;
        }
