...
/**
 * This class handles following admin commands: - cw_info = displays cursed weapon status -
 * cw_remove = removes a cursed weapon from the world, item id or name must be provided - cw_add =
 * adds a cursed weapon into the world, item id or name must be provided. Target will be the weilder
    ...
    if (command.startsWith("admin_cw_info")) {
      if (!command.contains("menu")) {
        ...
        for (CursedWeapon cw : cwm.getCursedWeapons()) {
          ...
          StringUtil.append(
              replyMSG, "<table width=270><tr><td>Name:</td><td>", cw.getName(), "</td></tr>");
