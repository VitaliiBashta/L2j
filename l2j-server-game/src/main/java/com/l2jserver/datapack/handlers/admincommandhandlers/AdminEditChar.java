...
public class AdminEditChar implements IAdminCommandHandler {
    ...
    // Karma control commands
      ...
      try {
        ...
        if (valid && (player.getClassId().getId() != classidval)) {
          ...
          String newclass =
              ClassListData.getInstance().getClass(player.getClassId()).getClassName();
          player.storeMe();
      ...
      try {
        ...
        if (player.isInParty()) {
          ...
          player
              .getParty()
              .broadcastToPartyMembers(player, PartySmallWindowDeleteAll.STATIC_PACKET);
  ...
  private void listCharacters(L2PcInstance activeChar, int page) {
    final L2PcInstance[] players =
        L2World.getInstance().getPlayersSortedBy(Comparator.comparingLong(L2PcInstance::getUptime));
    ...
    final PageResult result =
        HtmlUtil.createPage(
            ...
            20,
            i -> {
              return "<td align=center><a action=\"bypass -h admin_show_characters "
                  + i
                  + "\">Page "
                  + (i + 1)
                  + "</a></td>";
            ...
            player -> {
              ...
              sb.append("<tr>");
              sb.append(
                  "<td width=80><a action=\"bypass -h admin_character_info "
                      + player.getName()
                      + "\">"
                      + player.getName()
                      + "</a></td>");
    ...
    if (result.getPages() > 0) {
      html.replace(
          "%pages%",
          "<table width=280 cellspacing=0><tr>" + result.getPagerTemplate() + "</tr></table>");
    } else {
      html.replace("%pages%", "");
    }
  ...
  /**
   * Retrieve and replace player's info in filename htm file, sends it to activeChar as
   * NpcHtmlMessage.
   *
   * @param activeChar
   * @param player
  ...
  private void gatherCharacterInfo(L2PcInstance activeChar, L2PcInstance player, String filename) {
    ...
    adminReply.replace(
        ...
        String.valueOf(
            player.getClan() != null
                ? "<a action=\"bypass -h admin_clan_info "
                    + player.getObjectId()
                    + "\">"
    ...
    adminReply.replace(
        "%baseclass%", ClassListData.getInstance().getClass(player.getBaseClass()).getClientCode());
    ...
    adminReply.replace(
        "%access%",
        player.getAccessLevel().getLevel() + " (" + player.getAccessLevel().getName() + ")");
    ...
    adminReply.replace(
        ...
        player.getInstanceId() > 0
            ? "<tr><td>InstanceId:</td><td><a action=\"bypass -h admin_instance_spawns "
                + String.valueOf(player.getInstanceId())
                + "\">"
                + String.valueOf(player.getInstanceId())
                + "</a></td></tr>"
            : "");
  ...
  private void setTargetKarma(L2PcInstance activeChar, int newKarma) {
    ...
    if (newKarma >= 0) {
      ...
      SystemMessage sm =
          SystemMessage.getSystemMessage(SystemMessageId.YOUR_KARMA_HAS_BEEN_CHANGED_TO_S1);
      sm.addInt(newKarma);
      player.sendPacket(sm);
      // Admin information
      activeChar.sendMessage(
          "Successfully Changed karma for "
              + player.getName()
              + " from ("
              + oldKarma
              + ") to ("
  ...
  private void findCharacter(L2PcInstance activeChar, String CharacterToFind) {
    ...
    for (L2PcInstance player :
        L2World.getInstance()
            .getPlayersSortedBy(
                Comparator.comparingLong(
                    L2PcInstance::getUptime))) { // Add player info into new Table row
      name = player.getName();
  ...
  private void findCharactersPerIp(L2PcInstance activeChar, String IpAdress)
    ...
    for (L2PcInstance player :
        L2World.getInstance()
            .getPlayersSortedBy(Comparator.comparingLong(L2PcInstance::getUptime))) {
  ...
  private void findDualboxStrict(L2PcInstance activeChar, int multibox) {
    ...
    for (L2PcInstance player :
      ...
      IpPack pack =
          new IpPack(client.getConnection().getInetAddress().getHostAddress(), client.getTrace());
  ...
  private void gatherSummonInfo(L2Summon target, L2PcInstance activeChar) {
    ...
    html.replace(
        "%owner%", " <a action=\"bypass -h admin_character_info " + owner + "\">" + owner + "</a>");
    html.replace("%class%", target.getClass().getSimpleName());
    html.replace(
        "%ai%", target.hasAI() ? String.valueOf(target.getAI().getIntention().name()) : "NULL");
    html.replace(
        "%hp%", (int) target.getStatus().getCurrentHp() + "/" + target.getStat().getMaxHp());
    html.replace(
        "%mp%", (int) target.getStatus().getCurrentMp() + "/" + target.getStat().getMaxMp());
    ...
    if (target instanceof L2PetInstance) {
      html.replace(
          "%food%",
          ((L2PetInstance) target).getCurrentFed()
              + "/"
              + ((L2PetInstance) target).getPetLevelData().getPetMaxFeed());
      html.replace(
          "%load%",
          ((L2PetInstance) target).getInventory().getTotalWeight()
              + "/"
  ...
  private void gatherPartyInfo(L2PcInstance target, L2PcInstance activeChar) {
    ...
    for (L2PcInstance member : target.getParty().getMembers()) {
      if (color) {
        ...
        text.append(
            "<tr><td><table width=270 border=0 cellpadding=2><tr><td width=30 align=right>");
      }
      text.append(
          member.getLevel()
              + "</td><td width=130><a action=\"bypass -h admin_character_info "
              + member.getName()
              + "\">"
              + member.getName()
              + "</a>");
      text.append(
          "</td><td width=110 align=right>"
              + member.getClassId().toString()
              + "</td></tr></table></td></tr>");
      color = !color;
    }
