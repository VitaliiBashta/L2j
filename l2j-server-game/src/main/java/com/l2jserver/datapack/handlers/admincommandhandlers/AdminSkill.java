 ...
 *   <li>reset_skills
 *   <li>give_all_skills
 *   <li>give_all_skills_fs
 *   <li>admin_give_all_clan_skills
 *   <li>remove_all_skills
 *   <li>add_clan_skills
 *   <li>admin_setskill
 * </ul>
 */
...
public class AdminSkill implements IAdminCommandHandler {
  ...
  public boolean useAdminCommand(String command, L2PcInstance activeChar) {
    if (command.equals("admin_show_skills")) {
      ...
      activeChar.sendMessage(
          "You added yourself skill " + skill.getName() + "(" + id + ") level " + lvl);
    }
    return true;
  }
  ...
  private void adminGiveAllSkills(L2PcInstance activeChar, boolean includedByFs) {
    ...
    activeChar.sendMessage(
        "You gave "
            + player.giveAvailableSkills(includedByFs, true)
            + " skills to "
            + player.getName());
    player.sendSkillList();
  }
  ...
  private void adminGiveClanSkills(L2PcInstance activeChar, boolean includeSquad) {
    ...
    if (!player.isClanLeader()) {
      final SystemMessage sm =
          SystemMessage.getSystemMessage(SystemMessageId.S1_IS_NOT_A_CLAN_LEADER);
      sm.addString(player.getName());
      activeChar.sendPacket(sm);
    }

    final Map<Integer, L2SkillLearn> skills =
        SkillTreesData.getInstance().getMaxPledgeSkills(clan, includeSquad);
  ...
  private void adminRemoveSkill(L2PcInstance activeChar, int idval) {
    ...
    if (skill != null) {
      ...
      // Admin information
      activeChar.sendMessage(
          "You removed the skill " + skillname + " from " + player.getName() + ".");
      if (general().debug()) {
        _log.fine(
            "[GM]"
                + activeChar.getName()
  ...
  private void adminAddClanSkill(L2PcInstance activeChar, int id, int level) {
    ...
    activeChar.sendMessage(
        "You gave the Clan Skill: " + skillname + " to the clan " + clan.getName() + ".");
