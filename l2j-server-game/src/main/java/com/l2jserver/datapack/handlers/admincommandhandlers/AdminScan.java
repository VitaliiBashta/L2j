  ...
  private void sendNpcList(L2PcInstance activeChar, int radius) {
    ...
    for (L2Character character : activeChar.getKnownList().getKnownCharactersInRadius(radius)) {
      if (character instanceof L2Npc) {
        ...
        sb.append(
            "<td width=\"54\">"
                + Math.round(activeChar.calculateDistance(character, false, false))
                + "</td>");
        sb.append(
            "<td width=\"54\"><a action=\"bypass -h admin_deleteNpcByObjectId "
                + character.getObjectId()
                + "\"><font color=\"LEVEL\">Delete</font></a></td>");
        sb.append(
            "<td width=\"54\"><a action=\"bypass -h admin_move_to "
                + character.getX()
                + " "
                + character.getY()
                + " "
                + character.getZ()
                + "\"><font color=\"LEVEL\">Go to</font></a></td>");
        sb.append("</tr>");
      }
    }
