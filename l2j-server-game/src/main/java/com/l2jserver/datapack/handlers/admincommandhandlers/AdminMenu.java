...
public class AdminMenu implements IAdminCommandHandler {
  ...
  public boolean useAdminCommand(String command, L2PcInstance activeChar) {
    if (command.equals("admin_char_manage")) {
      ...
      try {
        ...
        teleportCharacter(
            player, activeChar.getLocation(), activeChar, "Admin is teleporting you.");
      ...
      try {
        ...
        for (L2PcInstance pm : player.getParty().getMembers()) {
          teleportCharacter(
              pm,
              activeChar.getLocation(),
              activeChar,
              "Your party is being teleported by an Admin.");
        }
      } catch (Exception e) {
      ...
      try {
        ...
        for (L2PcInstance member : clan.getOnlineMembers(0)) {
          teleportCharacter(
              ...
              activeChar.getLocation(),
              activeChar,
              "Your clan is being teleported by an Admin.");
        }
      } catch (Exception e) {
        _log.log(Level.WARNING, "", e);
      }
    ...
    if (target != null) {
      if (target instanceof L2PcInstance) {
        ...
        target.reduceCurrentHp(
            (target.getMaxHp() * customs().getChampionHp()) + 1, activeChar, null);
      } else {
  ...
  private void teleportCharacter(
      L2PcInstance player, Location loc, L2PcInstance activeChar, String message) {
    if (player != null) {
