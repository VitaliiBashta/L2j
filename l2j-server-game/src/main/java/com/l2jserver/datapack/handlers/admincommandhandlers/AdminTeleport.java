...
/**
 * This class handles following admin commands: - show_moves - show_teleport - teleport_to_character
 * - move_to - teleport_character
 */
@Service
public class AdminTeleport implements IAdminCommandHandler {
  ...
  public boolean useAdminCommand(String command, L2PcInstance activeChar) {
    ...
    if (command.equals("admin_show_moves_other")) {
      ...
      try {
        ...
        activeChar
            .getAI()
            .setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, new Location(x, y, z, 0));
      } catch (Exception e) {
        if (general().debug()) {
  ...
  private void teleportHome(L2PcInstance player) {
    ...
    player.teleToLocation(
        MapRegionManager.getInstance().getMapRegionByName(regionName).getSpawnLoc(), true);
    ...
    try (Connection con = ConnectionFactory.getInstance().getConnection();
      ...
      if (count == 0) {
        ...
        activeChar.sendMessage(
            "Player's [" + name + "] position is now set to (" + x + "," + y + "," + z + ").");
      }
  ...
  private void recallNPC(L2PcInstance activeChar) {
    ...
    if ((obj instanceof L2Npc)
        && !((L2Npc) obj).isMinion()
        && !(obj instanceof L2RaidBossInstance)
