...
public final class HallOfSuffering extends AbstractInstance {
  ...
  private void runTumors(HSWorld world) {
    for (int[] mob : getRoomSpawns(world.getStatus())) {
      final L2Npc npc =
          addSpawn(mob[0], mob[1], mob[2], mob[3], 0, false, 0, false, world.getInstanceId());
      world.npcList.put(npc, false);
    }

    final L2Npc mob =
        addSpawn(
            TUMOR_ALIVE, TUMOR_SPAWNS[world.getStatus()], false, 0, false, world.getInstanceId());
  ...
  private void bossSimpleDie(L2Npc boss) {
    ...
    // Send the Server->Client packet StatusUpdate with current HP and MP to all other L2PcInstance
    // to inform
    ...
    String htmltext =
        HtmCache.getInstance()
            .getHtm(
                ...
                "com/l2jserver/datapack/gracia/instances/SeedOfInfinity/HallOfSuffering/32530-10.htm");
  ...
  public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
    ...
    if (tmpworld instanceof HSWorld) {
      ...
      if (event.equalsIgnoreCase("spawnBossGuards")) {
        ...
        L2Npc mob =
            addSpawn(
                TWIN_MOBIDS[getRandom(TWIN_MOBIDS.length)],
                KLODEKUS_SPAWN,
                false,
                0,
                false,
    ...
    if (tmpworld instanceof HSWorld) {
      ...
      if (world.getStatus() < 5) {
        ...
        if (checkKillProgress(npc, world)) {
          ...
          && ((npc.getId() == KLODEKUS) || (npc.getId() == KLANIKUS))) {
    ...
    if (npc.getId() == TEPIOS) {
      ...
      if (((HSWorld) world).rewardItemId == -1) {
        ...
        return "32530-11.htm";
          ...
          && (player.getParty().getLeaderObjectId() == player.getObjectId())) {
  ...
  public String onTalk(L2Npc npc, L2PcInstance talker) {
    ...
    if (npc.getId() == MOUTH_OF_EKIMUS) {
      ...
      if (((HSWorld) world).rewardItemId == -1) {
        ...
        return "32530-11.htm";
      } else if ((talker.getParty() != null)
          && (talker.getParty().getLeaderObjectId() == talker.getObjectId())) {
        ((HSWorld) world).isRewarded = true;
        for (L2PcInstance member : talker.getParty().getMembers()) {
          final QuestState st =
              member.getQuestState(Q00695_DefendTheHallOfSuffering.class.getSimpleName());
