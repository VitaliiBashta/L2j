...
public class Beleth extends AbstractNpcAI {
  ...
  public Beleth(GrandBossManager grandBossManager, DoorData doorData) {
    ...
    addFirstTalkId(ELF);
    StatsSet info = this.grandBossManager.getStatsSet(REAL_BELETH);
    int status = grandBossManager.getBossStatus(REAL_BELETH);
    if (status == DEAD) {
      final long time = (info.getLong("respawn_time") - System.currentTimeMillis());
      if (time > 0) {
        startQuestTimer("BELETH_UNLOCK", time, null, null);
      } else {
        grandBossManager.setBossStatus(REAL_BELETH, ALIVE);
      }
    } else if (status != ALIVE) {
      grandBossManager.setBossStatus(REAL_BELETH, ALIVE);
    }
    this.doorData.getDoor(DOOR1).openMe();
  }
  ...
  public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon) {
    ...
    if ((distance > 500) || (getRandom(100) < 80)) {
      for (L2Npc beleth : _minions) {
        if ((beleth != null)
            && !beleth.isDead()
            && Util.checkIfInRange(900, beleth, attacker, false)
            && !beleth.isCastingNow()) {
      ...
      if ((_beleth != null)
          && !_beleth.isDead()
          && Util.checkIfInRange(900, _beleth, attacker, false)
          && !_beleth.isCastingNow()) {
  ...
  public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
    switch (event) {
      ...
      case "SPAWN2":
        {
          ZONE.broadcastPacket(
              new SpecialCamera(_camera1, 1800, -45, -45, 5000, 5000, 0, 0, 1, 0, 0));
      ...
      case "SPAWN3":
        {
          ZONE.broadcastPacket(
              new SpecialCamera(_camera1, 2500, -120, -45, 5000, 5000, 0, 0, 1, 0, 0));
      ...
      case "SPAWN4":
        {
          ZONE.broadcastPacket(
              new SpecialCamera(_camera2, 2200, 130, 0, 0, 1500, -20, 15, 1, 0, 0));
      ...
      case "SPAWN5":
        {
          ZONE.broadcastPacket(
              new SpecialCamera(_camera2, 2300, 100, 0, 2000, 4500, 0, 10, 1, 0, 0));
      ...
      case "SPAWN7":
        {
          ...
          ZONE.broadcastPacket(
              new SpecialCamera(_camera4, 900, 255, 0, 5000, 6500, 0, 10, 1, 0, 0));
      ...
      case "SPAWN9":
        {
          ZONE.broadcastPacket(
              new SpecialCamera(_camera4, 1000, 255, 0, 7000, 17000, 0, 25, 1, 0, 0));
      ...
      case "SPAWN15":
        {
          ZONE.broadcastPacket(
              new SpecialCamera(_camera3, 100, 270, 15, 3000, 6000, 0, 5, 1, 0, 0));
      ...
      case "SPAWN18":
        {
          ZONE.broadcastPacket(
              new SpecialCamera(_camera3, 700, 270, 20, 1500, 8000, 0, 0, 1, 0, 0));
      ...
      case "SPAWN26":
        {
          ...
          ZONE.broadcastPacket(new SpecialCamera(_camera1, 400, 290, 25, 0, 10000, 0, 0, 1, 0, 0));
          ZONE.broadcastPacket(
              new SpecialCamera(_camera1, 400, 110, 25, 4000, 10000, 0, 0, 1, 0, 0));
      ...
      case "SPAWN27":
        {
          ZONE.broadcastPacket(
              new SpecialCamera(_camera1, 400, 295, 25, 4000, 5000, 0, 0, 1, 0, 0));
      ...
      case "SPAWN28":
        {
          ZONE.broadcastPacket(
              new SpecialCamera(_camera1, 400, 295, 10, 4000, 11000, 0, 25, 1, 0, 0));
            ...
            for (L2Character charInside : ZONE.getCharactersInside()) {
              if (charInside != null) {
                if (charInside.isNpc()) {
                  ...
                  charInside.teleToLocation(
                      MapRegionManager.getInstance()
                          .getTeleToLocation(charInside, TeleportWhereType.TOWN));
                }
              }
            }
  ...
  public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon) {
    if (npc.getId() == REAL_BELETH) {
      ...
      grandBossManager.setBossStatus(REAL_BELETH, DEAD);
      final long respawnTime =
          (grandBoss().getIntervalOfBelethSpawn()
                  + getRandom(
                      -grandBoss().getRandomOfBelethSpawn(), grandBoss().getRandomOfBelethSpawn()))
              * 3600000;
      StatsSet info = grandBossManager.getStatsSet(REAL_BELETH);
      info.set("respawn_time", System.currentTimeMillis() + respawnTime);
      grandBossManager.setStatsSet(REAL_BELETH, info);
  ...
  public String onSkillSee(
    ...
    if (!npc.isDead()
        && (npc.getId() == REAL_BELETH)
        && !npc.isCastingNow()
        && skill.hasEffectType(L2EffectType.HP)
        && (getRandom(100) < 80)) {
  ...
  public String onEnterZone(L2Character character, L2ZoneType zone) {
    if (character.isPlayer() && (grandBossManager.getBossStatus(REAL_BELETH) == INIT)) {
      if (_priest != null) {
        _priest.deleteMe();
      }
      if (_stone != null) {
      ...
      grandBossManager.setBossStatus(REAL_BELETH, FIGHT);
