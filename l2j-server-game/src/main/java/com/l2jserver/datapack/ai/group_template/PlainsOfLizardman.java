...
public class PlainsOfLizardman extends AbstractNpcAI {
  ...
  // Misc
  // @formatter:off
  private static final int[] BUFF_LIST = {6, 7, 8, 11, 13};
  // @formatter:on
    ...
    switch (npc.getId()) {
      case TANTA_SUMMONER:
        if ((npc.getCurrentHp() < (npc.getMaxHp() * HP_PERCENTAGE)) && npc.isScriptValue(0)) {
          ...
          addAttackDesire(
              addSpawn(TANTA_SCOUT, npc.getX(), npc.getY(), npc.getZ(), 0, false, 0, false),
              attacker);
          addAttackDesire(
              addSpawn(TANTA_SCOUT, npc.getX(), npc.getY(), npc.getZ(), 0, false, 0, false),
              attacker);
              ...
              if ((monster.getId() == TANTA_MAGICIAN) || (monster.getId() == TANTA_SCOUT)) {
                ...
                target
                    ...
                    .setIntention(
                        ...
                        new Location(npc.getX(), npc.getY(), npc.getZ(), 0));
              }
            }
          }
