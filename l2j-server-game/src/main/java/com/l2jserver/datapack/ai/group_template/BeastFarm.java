...
public class BeastFarm extends AbstractNpcAI {
  ...
  public BeastFarm() {
    ...
    // Tamed beasts data
    TAMED_BEAST_DATA.add(
        new TamedBeast("%name% of Focus", new SkillHolder(6432, 1), new SkillHolder(6668, 1)));
    TAMED_BEAST_DATA.add(
        new TamedBeast("%name% of Guiding", new SkillHolder(6433, 1), new SkillHolder(6670, 1)));
    TAMED_BEAST_DATA.add(
        new TamedBeast("%name% of Swifth", new SkillHolder(6434, 1), new SkillHolder(6667, 1)));
    TAMED_BEAST_DATA.add(new TamedBeast("Berserker %name%", new SkillHolder(6671, 1)));
    TAMED_BEAST_DATA.add(
        new TamedBeast("%name% of Protect", new SkillHolder(6669, 1), new SkillHolder(6672, 1)));
    TAMED_BEAST_DATA.add(
        new TamedBeast("%name% of Vigor", new SkillHolder(6431, 1), new SkillHolder(6666, 1)));
  }
  ...
  public String onSkillSee(
    ...
    if (!Util.contains(FEEDABLE_BEASTS, npcId)
        || ((skillId != SKILL_GOLDEN_SPICE)
            && (skillId != SKILL_CRYSTAL_SPICE)
            && (skillId != SKILL_BLESSED_GOLDEN_SPICE)
            && (skillId != SKILL_BLESSED_CRYSTAL_SPICE)
            && (skillId != SKILL_SGRADE_GOLDEN_SPICE)
            && (skillId != SKILL_SGRADE_CRYSTAL_SPICE))) {
      return super.onSkillSee(npc, caster, skill, targets, isSummon);
    }
    ...
    int growthLevel =
        3; // if a mob is in FEEDABLE_BEASTS but not in _GrowthCapableMobs, then it's at max growth
    // (3)
    ...
    // If the mob is at 0th level (when it still listens to all feeders) lock it to the first
    // feeder!
    ...
    if (GROWTH_CAPABLE_MONSTERS.containsKey(npcId)) {
      // do nothing if this mob doesn't eat the specified food (food gets consumed but has no
      // effect).
      ...
      if (newNpcId == -1) {
        ...
        // if no, then do not allow a chance to raise the pet (food gets consumed but has no
        // effect).
        return super.onSkillSee(npc, caster, skill, targets, isSummon);
      }
  ...
  public void spawnNext(L2Npc npc, L2PcInstance player, int nextNpcId, int food) {
    ...
    if (Util.contains(TAMED_BEASTS, nextNpcId)) {
      final L2TamedBeastInstance nextNpc =
          new L2TamedBeastInstance(
              nextNpcId, player, food, npc.getX(), npc.getY(), npc.getZ(), true);
  ...
  private static class GrowthCapableMob {
    ...
    public int getLeveledNpcId(int skillId) {
      if (!_skillSuccessNpcIdList.containsKey(skillId)) {
        return -1;
      } else if ((skillId == SKILL_BLESSED_GOLDEN_SPICE)
          || (skillId == SKILL_BLESSED_CRYSTAL_SPICE)
          || (skillId == SKILL_SGRADE_GOLDEN_SPICE)
          || (skillId == SKILL_SGRADE_CRYSTAL_SPICE)) {
