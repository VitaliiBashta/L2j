...
public class CastleChamberlain extends AbstractNpcAI {
          ...
          if (npc.isMyLord(player)) {
            if (castle.getSiege().isInProgress()) {
              ...
              if ((SevenSigns.getInstance().getPlayerCabal(player.getObjectId())
                ...
                if (ticketCount
                    < (sevenSigns().getSevenSignsDawnTicketQuantity()
                        / sevenSigns().getSevenSignsDawnTicketBundle())) {
                  final NpcHtmlMessage html = getHtmlPacket(player, npc, "ssq_selldawnticket.html");
                  html.replace(
                      "%DawnTicketLeft%",
                      String.valueOf(
                          sevenSigns().getSevenSignsDawnTicketQuantity()
                              - (ticketCount * sevenSigns().getSevenSignsDawnTicketBundle())));
                  html.replace(
                      "%DawnTicketBundle%",
                      String.valueOf(sevenSigns().getSevenSignsDawnTicketBundle()));
                  html.replace(
                      "%DawnTicketPrice%",
                      String.valueOf(
                          sevenSigns().getSevenSignsDawnTicketPrice()
                              * sevenSigns().getSevenSignsDawnTicketBundle()));
                  player.sendPacket(html);
                } else {
              ...
              if ((SevenSigns.getInstance().getPlayerCabal(player.getObjectId())
                ...
                if (ticketCount
                  ...
                  if (player.getAdena() >= totalCost) {
                    ...
                    giveItems(
                        player,
                        sevenSigns().getSevenSignsManorsAgreementId(),
                        sevenSigns().getSevenSignsDawnTicketBundle());
  ...
  private String funcConfirmHtml(
  ...
  private int getFunctionFee(final int func, final int level) {
    ...
    switch (func) {
      case Castle.FUNC_RESTORE_EXP:
        fee =
            (level == 45)
                ? castle().getExpRegenerationFeeLvl1()
                : castle().getExpRegenerationFeeLvl2();
        break;
      case Castle.FUNC_RESTORE_HP:
        fee =
            (level == 300)
                ? castle().getHpRegenerationFeeLvl1()
                : castle().getHpRegenerationFeeLvl2();
        break;
      case Castle.FUNC_RESTORE_MP:
        fee =
            (level == 40)
                ? castle().getMpRegenerationFeeLvl1()
                : castle().getMpRegenerationFeeLvl2();
        break;
      ...
      case Castle.FUNC_TELEPORT:
        fee =
            (level == 1)
                ? castle().getTeleportFunctionFeeLvl1()
                : castle().getTeleportFunctionFeeLvl2();
        break;
    }
    return fee;
  }
