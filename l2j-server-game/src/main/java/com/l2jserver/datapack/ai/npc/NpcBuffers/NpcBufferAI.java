      ...
      case RANGE:
        {
          for (L2Character target :
              _npc.getKnownList().getKnownCharactersInRadius(skill.getAffectRange())) {
            final var affectObject = (AffectObject) _skillData.getAffectObject();
            switch (affectObject) {
              case FRIEND:
                {
