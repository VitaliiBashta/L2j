...
public class Baium extends AbstractNpcAI {
  ...
  private static final L2NoRestartZone zone =
      ZoneManager.getInstance().getZoneById(70051, L2NoRestartZone.class); // Baium zone
  // Status
      ...
      case "PLAYER_KILL":
        {
          if ((player != null) && player.isInsideRadius(npc, 16000, true, false)) {
            ...
            broadcastNpcSay(
                npc,
                Say2.NPC_ALL,
                NpcStringId.HOW_DARE_YOU_WAKE_ME_NOW_YOU_SHALL_DIE,
                player.getName());
  ...
  public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon) {
    if (zone.isCharacterInZone(killer)) {
      ...
      long respawnTime =
          (grandBoss().getIntervalOfBaiumSpawn()
                  + getRandom(
                      -grandBoss().getRandomOfBaiumSpawn(), grandBoss().getRandomOfBaiumSpawn()))
              * 3600000;
