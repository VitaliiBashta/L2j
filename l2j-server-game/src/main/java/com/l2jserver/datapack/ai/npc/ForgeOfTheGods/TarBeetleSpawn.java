...
public class TarBeetleSpawn extends IXmlReader {
  ...
  public void load() {
    ...
    if (!zones.isEmpty()) {
      //			spawnTask = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(() ->
      // zones.forEach(SpawnZone::refreshSpawn), 1000, 60000);
      //			shotTask = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(() ->
      // zones.forEach(SpawnZone::refreshShots), 300000, 300000);
    }
  }
