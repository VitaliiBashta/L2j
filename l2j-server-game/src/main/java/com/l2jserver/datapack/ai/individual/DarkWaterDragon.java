...
public class DarkWaterDragon extends AbstractNpcAI {
  ...
  private static final Set<Integer> SECOND_SPAWN =
      ConcurrentHashMap.newKeySet(); // Used to track if second Shades were already spawned
  private static Set<Integer> MY_TRACKING_SET =
      ConcurrentHashMap.newKeySet(); // Used to track instances of npcs
  private static Map<Integer, L2PcInstance> ID_MAP =
      new ConcurrentHashMap<>(); // Used to track instances of npcs
  ...
  public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon) {
    ...
    if (npcId == DRAGON) {
      ...
      } else if ((npc.getCurrentHp() < (npc.getMaxHp() / 2.0))
          && !(SECOND_SPAWN.contains(npcObjId))) {
    ...
    if (npc != null) {
      ...
      } else if (event.equalsIgnoreCase("4")) // spawns a detractor
      {
        addSpawn(DETRACTOR2, (npc.getX() - 100), (npc.getY() - 100), npc.getZ(), 0, false, 40000);
          ...
          "fafurion_despawn")) // Fafurion Kindred disappears and drops reward
      {
        ...
        npc.deleteMe();
          ...
          "fafurion_poison")) // Reduces Fafurions hp like it is poisoned
      {
  ...
  public String onSpawn(L2Npc npc) {
    ...
    if (npcId == FAFURION) {
      if (!MY_TRACKING_SET.contains(npcObjId)) {
        ...
        startQuestTimer(
            "fafurion_despawn", 120000, npc, null); // Fafurion Kindred disappears after two minutes
      }
    }
