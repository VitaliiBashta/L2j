...
public class DevastatedCastle extends ClanHallSiegeEngine {
  ...
  private static final double GUSTAV_TRIGGER_HP =
      NpcData.getInstance().getTemplate(GUSTAV).getBaseHpMax() / 12;
    ...
    synchronized (this) {
      ...
      if ((npc.getCurrentHp() < GUSTAV_TRIGGER_HP)
        ...
        broadcastNpcSay(
            ...
            Say2.NPC_ALL,
            NpcStringId
                .THIS_IS_UNBELIEVABLE_HAVE_I_REALLY_BEEN_DEFEATED_I_SHALL_RETURN_AND_TAKE_YOUR_HEAD);
        npc.getAI()
            .setIntention(
                CtrlIntention.AI_INTENTION_CAST, SkillData.getInstance().getSkill(4235, 1), npc);
      }
    }
  ...
  public String onSpawn(L2Npc npc) {
    if (npc.getId() == MIKHAIL) {
      broadcastNpcSay(
          npc,
          Say2.NPC_SHOUT,
          NpcStringId.GLORY_TO_ADEN_THE_KINGDOM_OF_THE_LION_GLORY_TO_SIR_GUSTAV_OUR_IMMORTAL_LORD);
    } else if (npc.getId() == DIETRICH) {
      broadcastNpcSay(
          npc, Say2.NPC_SHOUT, NpcStringId.SOLDIERS_OF_GUSTAV_GO_FORTH_AND_DESTROY_THE_INVADERS);
    }
    return null;
  }
