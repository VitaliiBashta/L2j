...
public class Race extends Event {
  ...
  public boolean eventStart(L2PcInstance eventMaker) {
    ...
    if (!general().customNpcData()) {
      ...
      eventMaker.sendMessage(
          "Event " + getName() + " can't be started because custom NPC table is disabled!");
      return false;
    }
    ...
    Broadcast.toAllOnlinePlayers(
        "Visit Event Manager in Dion village and signup, you have "
            + TIME_REGISTER
            + " min before Race Start...");

    // Schedule Event end
    _eventTask =
        ThreadPoolManager.getInstance()
            .scheduleGeneral(() -> StartRace(), TIME_REGISTER * 60 * 1000);
  ...
  protected void StartRace() {
    ...
    _eventTask =
        ThreadPoolManager.getInstance().scheduleGeneral(() -> timeUp(), TIME_RACE * 60 * 1000);
  }
  ...
  private L2Npc recordSpawn(
      int npcId, int x, int y, int z, int heading, boolean randomOffSet, long despawnDelay) {
