...
public class ListenerTest extends AbstractNpcAI {
  private static final int[] ELPIES = {20432, 22228};
    ...
    Containers.Global()
        .addListener(
            new ConsumerEventListener(
                Containers.Global(),
                EventType.ON_PLAYER_DLG_ANSWER,
                (OnPlayerDlgAnswer event) -> {
                  LOG.info(
                      ListenerTest.class.getSimpleName()
                          + ": "
                          + event.getActiveChar()
                          + " OnPlayerDlgAnswer: Answer: "
                          + event.getAnswer()
                          + " MessageId: "
                          + event.getMessageId());
                },
  ...
  /**
   * This method will be invoked as soon as an L2Attackable (Rabbits 20432 and 22228) is being
   * attacked from L2PcInstance (a player)
   */
  ...
  /**
   * This method will be invoked as soon as L2Attackable (Rabbits 20432 and 22228) are being killed
   * by L2PcInstance (a player)<br>
  ...
  public void onSiegeStart(OnCastleSiegeStart event) {
    LOG.info(
        getClass().getSimpleName()
            + ": The siege of "
            + event.getSiege().getCastle().getName()
            + " ("
            + event.getSiege().getCastle().getResidenceId()
            + ") has started!");
  }

  /**
   * This method will be invoked as soon as Ancient Adena (5575) item is created on player's
   * inventory (As new item!).<br>
  ...
  public void OnCreatureKill(OnCreatureKill event) {
    ...
    if ((event.getAttacker() != null)
      ...
      monster.dropItem(
          event.getAttacker().getActingPlayer(), new ItemHolder(57, getRandom(100, 1000)));
    }
  }
  ...
  public void onPlayerLogin(OnPlayerLogin event) {
    LOG.info(
        getClass().getSimpleName() + ": Player: " + event.getActiveChar() + " has logged in!");
  }

  /**
   * Prioritized event notification - Ensuring that this listener will be the first to receive
   * notification.<br>
   * Also this method interrupts notification to other listeners and taking over return if somehow
   * it wasn't the first one to set.<br>
