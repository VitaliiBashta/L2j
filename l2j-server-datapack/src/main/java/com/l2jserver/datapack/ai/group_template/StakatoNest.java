...
public final class StakatoNest extends AbstractNpcAI {
  ...
  public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon) {
    ...
    if ((mob.getId() == STAKATO_LEADER)
        && (getRandom(1000) < 100)
        && (mob.getCurrentHp() < (mob.getMaxHp() * 0.3))) {
  ...
  public String onSkillSee(
    ...
    if (Util.contains(COCOONS, npc.getId())
        && targets.contains(npc)
        && (skill.getId() == GROWTH_ACCELERATOR)) {
      npc.doDie(caster);
      final L2Npc spawned =
          addSpawn(
              STAKATO_CHIEF,
              ...
              npc.getY(),
              npc.getZ(),
              Util.calculateHeadingFrom(npc, caster),
              false,
              0,
              true);
  ...
  public String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
    ...
    if (npcId > 0) {
      ...
      npc.deleteMe();
      final L2Npc spawned =
          addSpawn(npcId, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), false, 0, true);
